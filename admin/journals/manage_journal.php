    ...
    <form action="" id="journal-form">
        ...
        <div class="row">
            <div class="form-group col-md-6">
                <label for="account_id" class="control-label">Journal Type</label>
                <select id="account_id" class="from-control form-control-sm form-control-border select2">
                    <option value="" disabled selected></option>
                    <?php 
                    $accounts = $conn->query("SELECT * FROM `journal_list` where delete_flag = 0 and `status` = 1 order by `name` asc ");
            ...
            <div class="form-group col-md-6">
                <label for="group_id" class="control-label">Chart Account Type</label>
                <select id="group_id" class="from-control form-control-sm form-control-border select2">
                    <option value="" disabled selected></option>
                    <?php 
                    $groups = $conn->query("SELECT * FROM `chart_account_list` where delete_flag = 0 and `status` = 1 order by `name` asc ");
        ...
        <table id="account_list" class="table table-stripped table-bordered">
            ...
            <thead>
                <tr class="bg-gradient-primary">
                    ...
                    <th class="text-center">Chart Account</th>
            ...
            <tbody>
                <?php 
                if(isset($id)):
                $jitems = $conn->query("SELECT j.*,a.name as account, g.name as `group`, g.type FROM `journal_items` j inner join journal_list a on j.account_id = a.id inner join chart_account_list g on j.group_id = g.id where journal_id = '{$id}'");
