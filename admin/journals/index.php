...
<style>
    ...
    th div:last-child, td div:last-child {
    border-right: none;
    }
	th, .col-6, .col-3 {
...
<div class="card card-outline card-primary">
	<div class="card-header">
		...
		<div class="card-tools">
			<button class="btn btn-primary btn-flat btn-sm" id="create_new" type="button" style="border-radius: 5px;"><i class="fa fa-pen-square"></i> Add New Journal Entry</button>
		</div>
	</div>
	<div class="card-body
	">
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
					...
					$users = $conn->query("SELECT id,username FROM `user_admin` where id in (SELECT `user_id` FROM `journal_entries` {$swhere})");
					...
					<tr>
						...
						<td class="p-0">
							...
							<?php 
							$jitems = $conn->query("SELECT j.*,a.name as account, g.type as `type` FROM `journal_items` j inner join journal_list a on j.account_id = a.id inner join chart_account_list g on j.group_id = g.id where j.journal_id = '{$row['id']}'");
							while($rowss = $jitems->fetch_assoc()):
							?>
							<div class="d-flex w-100">
								<div class="col-6"><span class="pl-4"><?= $rowss['account'] ?></span></div>
