...
<style>
    ...
    .callout.border-primary.shadow.rounded-0{
    background-color: #edfaef; /* very light green */
    padding: 30px;
    }
	.card-body{
    ...
    #print:hover {
    background: linear-gradient(to bottom, #ff3333 0%, #000000 100%);
}
</style>
        ...
        <div class="container-fluid" id="outprint">
			...
			<table class="table table-hover table-bordered">
                ...
                <tbody>
                    ...
                    $accounts = $conn->query("SELECT * FROM `journal_list`  where id in (SELECT account_id FROM `journal_items` where journal_id in (SELECT id FROM `journal_entries` where date(journal_date) BETWEEN '{$from}' and '{$to}' ))");
                    while($arow = $accounts->fetch_assoc()):
                        $items = $conn->query("SELECT j.*,g.type FROM `journal_items` j inner join chart_account_list g on j.group_id = g.id where j.account_id = '{$arow['id']}' and j.journal_id in (SELECT id FROM `journal_entries` where date(journal_date) BETWEEN '{$from}' and '{$to}' )");
                    ?>
                    <tr>
